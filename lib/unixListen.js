  ...
  function create(url, params) {
    ...
    var m = map({
      ...
      connection: function (e) {
        ...
        var u = URL.parse(addr,true, true) 
        if(u.auth && !e.address && e.remote)
        e.remoteAddress = u.format().replace(u.protocol+"//",url.protocol+"//"+encodeURIComponent(util.encode(e.remote, params.encoding || 'base64'))+"@")
        else e.remoteAddress = u.format()
        return e
      },
      message: function (e) {
        e.protocol = url.protocol
        return {
          ...
          data: e.data,
          protocol: e.protocol,
          remoteAddress: util.toRemoteAddress(e,params.encoding)
        }
      }
    })
