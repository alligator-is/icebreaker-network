...
function Network(params) {
  ...
  _(listen(), _.drain(function (e) {
    if (e.event === 'started') {
      ...
      var conn = e.args
      _(conn,self._handshake(function(stream,id){
        console.log('id',id,conn.direction);
          _(["hallo"],stream,_.drain(function(){
            console.log('hallo');
          }))
      }),conn);
    }
...
proto._handshake = function(cb){
  var stream = handshake(function (err) {
    if (err) console.error(err.message||err)
  })
  var shake = stream.handshake
  var l = Buffer.isBuffer(this.id)?this.id:new Buffer(this.id);
