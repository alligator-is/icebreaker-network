...
function Network(params){
  var self =this;
	...
	t.on('started',function(){
		self._notify({event:'started',args:t});
	});
	t.on('stopped',function(){
		self._notify({event:'stopped',args:t});
	});
	t.on('response',function(msg){
		self._notify({event:'response',args:msg});
	});
	t.on('connection',function(connection){
		self._notify({event:'connection',args:connection});	
	});
...
proto.start = function(){
	var self = this;
	this.transports.forEach(function(t){
		self._notify({event:'start',args:t})
		if(t.start)t.start();
	});	
}
...
proto.stop = function(){
	var self = this;
	this.transports.forEach(function(t){
		self._notify({event:'stop',args:t})
		if(t.stop)t.stop();
	});	
}
...
var network = new Network({transports:[PeerNet(),UDP({loopback:true})]});
require('./lib/discovery')(network,{interval:2000});
network.start();
